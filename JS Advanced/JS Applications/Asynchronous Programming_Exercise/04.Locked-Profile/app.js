function lockedProfile() {
    ...
    (async () => {
        ...
        let randomprofile = document.querySelector('.profile');
        randomprofile.remove();

        Object.keys(profiles).forEach((key, i) => {
            let currProfile = profiles[key];
            let createCurrProfile = createProfile(i + 1, currProfile.username, currProfile.email, currProfile.age);

            mainSection.appendChild(createCurrProfile);
        })
    ...
    function createProfile(userIndex, username, email, age) {
        ...
        showMoreButton.addEventListener('click', showHideInfo);
